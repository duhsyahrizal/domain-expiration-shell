...
check_domain_status()
{
    ...
    elif [ "${TLDTYPE}" == "id" ]; # added by duhsyahrizal 22-mar-2021
    then
            REGISTRAR=`cat ${WHOIS_TMP} | ${AWK} -F: '/Sponsoring Registrar Organization:/ && $0 != "" { REGISTRAR=substr($0,36,80) } END { print REGISTRAR }'`
    elif [ "${TLDTYPE}" == "info" ];
    then
        REGISTRAR=`cat ${WHOIS_TMP} | ${AWK} -F: '/Registrar:/ && $2 != ""  { REGISTRAR=substr($2,2,17) } END { print REGISTRAR }'`
    elif [ "${TLDTYPE}" == "ca" ];
    ...
    then
	    ...
	    DOMAINDATE=`echo $tday-$tmonth-$tyear`
    elif [ "${TLDTYPE}" == "id" ]; # added by duhsyahrizal
    then
            tdomdate=`cat ${WHOIS_TMP} | ${AWK} '/Expiration Date:/ { print $3 }'`
            tyear=`echo ${tdomdate} | ${CUT} -d'-' -f1`
            tmon=`echo ${tdomdate} | ${CUT} -d'-' -f2`
